...
class Trainer(object):
    ...
    def train(self, param):
        ...
        # dataset, BUFFER_SIZE, self.max_length_src, self.max_length_trg = to_tf_dataset(param.train, param.batch_size, param.encoder_src, param.encoder_trg)
        ...
        for epoch in range(param.epochs):
            ...
            for batch_idx, (src, trg) in gen_batch(param.train, param.encoder_src, param.encoder_trg, param.batch_size):
                ...
                if (batch_idx + 1) % param.evaluate_frequency == 0:
                    ...
                    bleu = Trainer.evaluate(self.encoder, self.decoder, param,
                                            param.max_length_src, param.max_length_trg, epoch, batch_idx)
                    if best_bleu < bleu:
                        best_bleu = bleu
    ...
    def train_step(self, src, trg, enc_hidden):
        ...
        with tf.GradientTape() as tape:
            ...
            dec_input = tf.expand_dims([SOS] * enc_output.shape[0], 1)  # encoder_trg.encode('<s>')
